...
class RegistrationViewModel : ViewModel() {
        ...
        firebaseAuth.createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    user?.let {
                        sendEmailVerification(it, firstName, lastName, userType, businessName, businessLocation, locationName)
                    }
                } else {
    ...
    private fun sendEmailVerification(user: FirebaseUser, firstName: String, lastName: String, userType: String, businessName: String?, businessLocation: String?, locationName: String?) {
        user.sendEmailVerification()
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    _emailVerificationStatus.value = "Verification email sent! Please check your inbox."
                    checkEmailVerification(user, firstName, lastName, userType, businessName, businessLocation, locationName)
                } else {
    ...
    private fun checkEmailVerification(user: FirebaseUser, firstName: String, lastName: String, userType: String, businessName: String?, businessLocation: String?, locationName: String?) {
        ...
        val runnable = object : Runnable {
            override fun run() {
                ...
                if (user.isEmailVerified) {
                    ...
                    saveUserToFirestore(
                        userData,
                        if (userType == "Hauler Business Admin") businessName else null,
                        if (userType == "Hauler Business Admin") businessLocation else null,
                        if (userType == "Hauler Business Admin") locationName else null
                    )
                } else {
    ...
    private fun saveUserToFirestore(user: User, businessName: String?, businessLocation: String?, locationName: String?) {
        ...
        if (user.userType == "Hauler Business Admin") {
            ...
            userMap["location"] = businessLocation // Add long, lat
            userMap["locationName"] = locationName
        }
